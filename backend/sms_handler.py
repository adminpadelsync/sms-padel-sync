from twilio_client import (
    send_sms, set_reply_from, set_club_name, get_club_name, 
    set_dry_run, get_dry_run_responses
)
...
def handle_incoming_sms(from_number: str, body: str, to_number: str = None, dry_run: bool = False, history: List[Dict[str, str]] = None, golden_samples: List[Dict[str, Any]] = None):
    ...
    # Initialize defaults for dry_run return capture
    intent = "UNKNOWN"
    confidence = 0.0
    entities = {}
    raw_reasoning = ""
    reasoner_result = ReasonerResult(intent, confidence, entities)
    ...
    try:
        ...
        # START_MATCH and JOIN_GROUP should not nuke the state if we are already in a match setup flow
        ...
        if player and not current_state:
            ...
            if all_sent_invites:
                ...
                if invite_index is not None:
                    if invite_index < 0 or invite_index >= len(all_sent_invites):
                        send_sms(from_number, f"❌ Invalid match number. You have {len(all_sent_invites)} pending invite(s). Reply 1-{len(all_sent_invites)}.", club_id=club_id)
                        return
            ...
            if cmd == "reset":
                ...
                send_sms(from_number, "📊 Reply MATCHES to see your match invites with details.", club_id=club_id)
                return
                ...
                return
            elif intent in ["GREETING", "CHITCHAT"] and reasoner_result.reply_text:
                send_sms(from_number, reasoner_result.reply_text, club_id=club_id)
                return
            else:
                # Default welcome or help
                reply = reasoner_result.reply_text or msg.MSG_WELCOME_BACK.format(club_name=club_name, name=player['name'])
                send_sms(from_number, reply, club_id=club_id)
                return
        ...
        if not current_state:
            ...
            send_sms(from_number, msg.MSG_WELCOME_NEW.format(club_name=club_name, booking_system=booking_system_display), club_id=club_id)
        ...
        if current_state in [msg.STATE_WAITING_NAME, msg.STATE_WAITING_LEVEL, msg.STATE_WAITING_GENDER, msg.STATE_WAITING_AVAILABILITY, msg.STATE_WAITING_GROUPS_ONBOARDING]:
            ...
            if not nums:
                if intent in ["GREETING", "CHITCHAT", "UNKNOWN"]:
                     send_sms(from_number, msg.MSG_NUDGE_GROUP_SELECTION, club_id=club_id)
                     return
        ...
        # --- Match Request Flow ---
            ...
            try:
                ...
                send_sms(from_number, "Sorry, something went wrong processing your request. Please try again.", club_id=club_id)
                clear_user_state(from_number)
            ...
            try:
                ...
                send_sms(from_number, "Sorry, something went wrong confirming your match. Please text PLAY to try again.", club_id=club_id)
                clear_user_state(from_number)
        ...
        # 6. RETURN CAPTURED RESPONSES (For Dry Run)
        if dry_run:
            captured = get_dry_run_responses()
            set_dry_run(False) # Reset
            # Note: We return inside finally to override any early returns in the try block
            return {
                "responses": captured,
                "intent": intent,
